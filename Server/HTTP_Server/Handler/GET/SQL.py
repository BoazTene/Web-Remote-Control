...
class DataBase:
    ...
    def get_spec_data(self, variable: str, value: str, table: str):
        data = []

        try:
            for row in self.c.execute("SELECT * FROM {0} WHERE {1}='{2}'".format(str(table), str(variable), str(value))):
                data.append(row)
    ...
    def add_raw(self, key, user_name, password, client):
